...
STALL_EPSILON_RPM = 4 # 윈도우 평균 RPM이 이 값 미만이면 stall(접촉)로 판정 (§12-1 시뮬 값과 동일, 완전 0 요구 X)
...
MAX_RUN_TIME_S = 600.0  # 전체 실행 시간 제한 [s] — 10분 경과 시 자동 정지
...
def motor_control(motor, run_event, input_queue, rpm_log, timeout=0.05):
    ...
    rpm_log: 매 tick (경과시간, RPM, 상태, 크랭크각)을 추가하는 리스트.
             호출부에서 만들어 넘겨주면 종료(타임아웃/이탈) 후에도 CSV로 저장 가능.
    """
    ...
    while run_event.is_set():
        now = time.time()
        if now - t0 >= MAX_RUN_TIME_S:
            print(f"[TIMEOUT] 최대 실행시간 {MAX_RUN_TIME_S/60:.0f}분 경과 → 정지")
            motor.set_enable(0)
            motor.set_speed(0)
            run_event.clear()
            break
        ...
        rpm_log.append((now - t0, rpm_val, state, crank_deg))
        ...
        if state == "STRIKE":
            ...
            # 접촉: 짧은 윈도우 평균 RPM이 임계치 미만 (grace 이후) → RETRACT (§12-4)
            stalled = len(rpm_buffer) == STALL_SAMPLES \
                and (sum(rpm_buffer) / STALL_SAMPLES) < STALL_EPSILON_RPM
...
if __name__ == "__main__":
    ...
    run_event.set()
    input_queue = Queue()
    rpm_log = []   # (elapsed_s, rpm, state, crank_deg) — 종료 시 CSV로 저장

    motor_thread = threading.Thread(
        target=motor_control, args=(motor, run_event, input_queue, rpm_log, global_timeout[0])
    )
    input_thread = threading.Thread(
        target=user_input_handler, args=(motor, run_event, input_queue), daemon=True
    )
    ...
    motor_thread.join()
    # input_thread는 daemon: input()에서 블로킹 중이면 join()이 무한 대기하므로
    # 여기서 join하지 않고 프로세스 종료 시 자동 정리되도록 둔다.
    motor.close()

    csv_path = f"rpm_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["elapsed_s", "rpm", "state", "crank_deg"])
        writer.writerows(rpm_log)
    print(f"[LOG] RPM {len(rpm_log)}개 샘플 저장 → {csv_path}")
